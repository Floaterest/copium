// 2022-10-29 https://atcoder.jp/contests/abc248/tasks/abc248_d
...
fn bound(arr: &[usize], lower: usize, upper: usize) -> usize {
    use std::convert::identity;
    let left = arr.binary_search(&lower).unwrap_or_else(identity);
    let right = arr.binary_search(&(upper + 1)).unwrap_or_else(identity);
    right - left
}
...
fn solve<R: Read, W: Write>(mut re: Reader<R>, mut wr: Writer<W>) {
    ...
    wr.nn((0..re.u()).map(|_| r!(re,usize,usize,usize)).map(
        |(l, u, x)| bound(&g[x], l, u)
    ))
}
