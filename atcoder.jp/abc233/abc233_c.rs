...
fn solve<R: Read, W: Write>(mut re: Reader<R>, mut wr: Writer<W>) {
    let (n, prod) = r!(re,usize,usize);
    let a: Vec<Vec<_>> = (0..n).map(|_| r!(re,[usize;re.us()]).collect()).collect();
    let mut s = vec![(0, 1)];
    let mut t = 0;

    while let Some((i, p)) = s.pop() {
        if i == n {
            t += (p == prod) as usize;
            continue;
        }
        s.extend(a[i].iter().filter(|&&m| m <= prod / p).map(|&m| (i + 1, m * p)));
    }
