...
pub mod cp {
    ...
    pub mod reader {
        ...
        impl<R: Read> Reader<R> {
            ...
            pub fn ii(&mut self) -> i64 { self.next::<i64>() }
        ...
        macro_rules! read {
            // read iter, e.g. re!(r, [i32;20])
            ($r:expr, [$type:ty; $len:expr]) => ((0..$len).map(|_| $r.next::<$type>()));
...
fn solve<R: Read, W: Write>(mut re: Reader<R>, mut wr: Writer<W>) {
    let (a, b) = (re.us() - 1, re.us());
    let s = re.bytes();
    wbn!(wr, &s[..a], &s[a..b].iter().rev().cloned().collect::<Vec<_>>(), &s[b..]);
}
