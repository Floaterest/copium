// 2023-04-23 https://atcoder.jp/contests/abc298/tasks/abc298_b
...
fn rotate(m: &Vec<Vec<bool>>, n: usize) -> Vec<Vec<bool>> {
    let mut res = vec![vec![false; n]; n];
    m.iter()
        .enumerate()
        .for_each(|(i, row)| row.iter().enumerate().for_each(|(j, x)| res[n - 1 - j][i] = *x));
    res
}

fn solve<R: Read, W: Write>(mut re: Reader<R>, mut wr: Writer<W>) {
    let n = re.u();
    let mut a: Vec<Vec<_>> = (0..n).map(|_| r!(re, [u8; n]).map(|n| n == 1).collect()).collect();
    let b: Vec<Vec<_>> = (0..n).map(|_| r!(re, [u8; n]).map(|n| n == 1).collect()).collect();
    let mut y = false;
    for _ in 0..4 {
        y = a
            .iter()
            .enumerate()
            .all(|(i, row)| row.iter().enumerate().all(|(j, x)| !*x || b[i][j]));
        if y {
            break;
        }
        a = rotate(&a, n);
    ...
    wr.y(y);
    // wr.y(once(a.clone())
    //     .chain((0..3).scan(a, |m, _| {
    //         *m = rotate(m, n);
    //         Some(m.clone())
    //     }))
    //     .any(|m| {
    //         m.iter().enumerate().all(|(i, row)| row.iter().enumerate().all(|(j, x)| !*x || b[i][j]))
    //     }));
}
