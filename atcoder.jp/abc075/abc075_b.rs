...
fn solve<R: Read, W: Write>(mut re: Reader<R>, mut wr: Writer<W>) {
    let mut map = [[b'_'; 52]; 52];
    let (h, w) = re!(re,usize,usize);
    (1..=h).for_each(
        |y| re.bytes().iter().enumerate().for_each(|(x, b)| map[y][x + 1] = *b)
    );
    map[..=h].iter().enumerate().skip(1).for_each(
        |(y, r)| wr.wn((*r)[..=w].iter().enumerate().skip(1).map(
            |(x, b)| if b == &b'#' { '#' } else {
                (d8.iter().map(
