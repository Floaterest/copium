...
inline namespace Output{
    ...
    void yneos(bool yes){ ps(yes?"YES":"NO"); }
}
//#endregion namespace
...
int ca, cb, cn;
bool solve(char s[][5], int n){
    f0r(a, 4){
        FOR(b, a + 1, 5){
            ca = cb = cn = 0;
            f0r(i, n){
                if(s[i][a] == '1' and s[i][b] == '0') ca++;
                if(s[i][a] == '0' and s[i][b] == '1') cb++;
                if(s[i][a] == '0' and s[i][b] == '0') cn++;
            }
            if(not cn and ca * 2 <= n and cb * 2 <= n) return true;
        }
    }
    return false;
}
